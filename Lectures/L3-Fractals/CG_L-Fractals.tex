...
\begin{frame}{Введение}
	...
	\begin{figure}
		\includegraphics[width=0.9\textwidth]{images/Nature.png}
		\caption{Объекты живой природы}
	\end{figure}

	\note{
		\textit{Воображение иссякает прежде природы.}
...
\begin{frame}{История фракталов}
	...
	\note{
		...
		Бенуа Б. Мандельброт
		является
		создателем термина \textquotedbl фрактал\textquotedbl~ и \textquotedbl фрактальной геометрии\textquotedbl

		Термин \textquotedbl Фрактал\textquotedbl~ появился 1975 г.

		Произошло от
		лат. слова fractus --- фрагментированный, дробный, ломанный.

		\textquotedbl Фракталом называется множество, размерность Хаусдорфа-Безиковича которого строго больше его топологической размерности\textquotedbl

		\textquotedbl Все фигуры, которые я исследовал и назвал фракталами, в моем представлении обладали свойством быть нерегулярными, но самоподобными\textquotedbl

		Мандельброт Б. \textquotedbl Геометрия природы\textquotedbl.
...
\begin{frame}{Эффект Ричардсона}{Парадокс береговой линии (1950-е годы)}
	\begin{figure}
		\includegraphics[width=0.85\textwidth]{images/shoreline.jpg}
	...
	\note{
		...
		$L$ --- длина кривой, проходящей через две точки;

		$l$ --- длина шага, с помощью которого приблизительно пытаемся измерить расстояние;

		$N$ --- количество отрезков;

		$lN$ --- длина ломанного отрезка;
...
\begin{frame}{Фрактальная размерность}
	...
	\note{
		...
		Примечание.
		Если $d  = \{1, 2, 3 \dots \}$, т.е. $d \in N$, то величина $d$ характеризует евклидову размерность.
		%{0} \cup
...
\begin{frame}{Канторово множество} {(канторов дисконтинуум, канторова пыль)}
	...
	\]
	\[
		E_1=\bigg[ 0,\frac{1}{3}\bigg] \cup \bigg[\frac{2}{3},1\bigg]
	\]
	\[
		E_2
		=
	...
	Получается $N=2^n$ сегментов, где $l_n=\frac{1}{3^n}$.
	}
\end{frame}
...
\begin{frame}{Салфетка Серпинского (треугольник Серпинского)}
	...
	\note{
		...
		\[
			=
			\log_{2} 3 \approx 1.58496250072  < 2
		\]
...
\begin{frame}{L-systems (L-системы)}
	...
	\note{
		Такой способ построения предложил в 1968 г. Аристид Линденмайер, венгерский биолог и ботаник из Утрехтского университета. Он использовал L-системы для описания поведения клеток растений и моделирования процесса развития растения.
	}
\end{frame}
...
\begin{frame}{Стохастические фракталы}
	\begin{figure}
		\includegraphics[width=0.85\textwidth]{images/stochastic-fractal-1.png}
...
\begin{frame}{Построение салфетки Серпинского с помощью метода случайных итераций}
	\begin{figure}
		...
		\caption{Игра \textquotedbl Хаос\textquotedbl (слева) и алгоритм построения (справа)}
	\end{figure}
	\note{
...
\begin{frame}{Стохастические фракталы}
	\begin{figure}
		\includegraphics[width=0.95\textwidth]{images/stochastic-fractal-2.png}
