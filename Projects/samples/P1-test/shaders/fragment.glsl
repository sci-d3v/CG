...
/*
    ...
    color - цвет звезды.
*/
vec4 Star(vec2 pos, vec4 color, vec2 p0, float r, float R, float angle)
{
    ...
    if(pow(pos.x-p0.x,2.0) + pow(pos.y-p0.y,2.0) < pow(R,2.0))
    {
        ...
        for(int i = 0; i < count; ++i)
        {
            ...
            for(int j = 0;  j< 3; ++j)
            {
                ...
                // в зависимости от числа вершин, порядкого номера и общего угла
                float angle_ = angle + float(index)*PI/float(count);
                // Позиция смещения, относительно центра звезды
                vec2 shift = vec2(mod(float(index),2.0)*r+
                                  mod(float(index+1),2.0)*R).xy*
...
void main()
{
    ...
    gl_FragColor = Star(position, vec4(1,0,0,1), vec2(0.5), 0.15, 0.4, 0.25*PI);
}