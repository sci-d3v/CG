...
int main()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(constellation), constellation, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        GLfloat angleArray[]={-0.03f,   0.08f,  -0.03f, 0.08f,  0.03f,  -0.05f, -0.06f};
        for (int position = 0; position < 7; ++position)
        {

            glUniform3f(translationLocation, constellation[position * 6], constellation[position * 6 + 1],
                        constellation[position * 6 + 2]);
            glUniform3f(scaleLocation, scaleArray[position], scaleArray[position], 1.0f);
